...
save_dir_gen = "Generated_valid"
tl.files.exists_or_mkdir(save_dir_gen)
save_dir1 = "Generated_train"
tl.files.exists_or_mkdir(save_dir1)
save_dir2 = "Adversarial learning"
tl.files.exists_or_mkdir(save_dir2)
...
def train():
    ...
    for epoch in range(n_epoch):
        for step, (lr_patchs, hr_patchs) in enumerate(train_ds):
			...
			if (epoch == n_epoch):
				tl.vis.save_images(fake_patchs.numpy(), [2, 4], os.path.join(save_dir2, 'train_g_{}.png'.format(epoch)))
        ...
        if (epoch != 0) and (epoch % 10 == 0):
            ...
            G.save_weights(os.path.join(checkpoint_dir, 'g1.h5'))
            D.save_weights(os.path.join(checkpoint_dir, 'd1.h5'))
...
def evaluate():
	...
	for i in out:
		...
		print("[*] save images")
		tl.vis.save_image(out[i], os.path.join(save_dir_gen, 'valid_gen{}.png'.format(i)))
		tl.vis.save_image(valid_lr_img[i], os.path.join(save_dir, 'valid_lr{}.png'.format(i)))
		tl.vis.save_image(valid_hr_img, os.path.join(save_dir, 'valid_hr{}.png'.format(i)))

		out_bicu = scipy.misc.imresize(valid_lr_img[i], [size[0] * 4, size[1] * 4], interp='bicubic', mode=None)
		tl.vis.save_image(out_bicu, os.path.join(save_dir, 'valid_bicubic{}.png'.format(i)))
